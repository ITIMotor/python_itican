...
class ITICANChannel:
    ...
    def get_channel(chn_container: list, chn_index: str):
        ...
        if result == 0:
            chn_container.append(ITICANChannel(chn_ptr))
        return result
    ...
    def set_fd_baud_rate(self, baud_rate_input):
        """
        设置数据段波特率
        :param baud_rate_input: uint64 ; 数据段波特率参数
        :return: getLastError 错误码
        """
    ...
    def set_custom_baud_rate(self, custom_baud_rate_str_input: str):
        """
        设置自定义波特率参数
        :param custom_baud_rate_str_input: str ; 自动波特率参数的字符串形式
        :return: getLastError 错误码
        """
        str_temp = ctypes.create_string_buffer(500)
        str_temp.value = str(custom_baud_rate_str_input).encode('utf-8')
        result = importDLL.dll.setCustomBaudRate(self.chn_pointer, str_temp)
        return result
    ...
    def set_message(self, one_message: CANMessage, timeout=0):
        ...
        result = importDLL.dll.setMessage(self.chn_pointer, can_id_, message_type_, is_extended_, data_array_, date_length_,
                                timeout_)
        return result
    ...
    def get_message(self, one_message_output: list, timeout=0):
        ...
        transmitted_temp = ctypes.c_uint64(0)  # no output
        timestamp_temp = ctypes.c_uint64(0)
        ...
        result = importDLL.dll.getMessage(self.chn_pointer, ctypes.byref(id_temp), ctypes.byref(message_type_temp),
                                ...
                                ctypes.byref(timestamp_temp), data_temp, ctypes.byref(data_l_temp), timeout_temp)
        if result == 0:
    ...
    def instance_test(self):
        print("this is a instance call")


# def pyw_get_channel(chn_name)
